...
def iterative_learning_control(m, K, max_iterations=200, threshold=0.1):
    ...
    control_input_y = 0
    errors = []
    pid_x = PIDController(Kp=0.015, Ki=0.0, Kd=0.005)
    pid_y = PIDController(Kp=0.015, Ki=0.0, Kd=0.005)
    pid_z = PIDController(Kp=0.015, Ki=0.0, Kd=0.005)
    ...
    for iteration in range(max_iterations):
        ...
        if abs(control_input_z) >= cap_value:
            control_input_z = cap_value if control_input_z_rot > 0 else -cap_value
...
iterative_learning_control(m=m, K=K)
