...
class BayesianController:
    def __init__(self, demo_image_path, stablizing_pose, batch_size=5):
        ...
        self.optimizer = BayesianOptimization(
            ...
            pbounds={'delta_x': (-0.1, 0.1), 'delta_y': (-0.1, 0.1), 'delta_yaw': (-45, 45)},
            random_state=1,
    ...
    def observe(self, timeout=1):
        ...
        rospy.sleep(1)
        rgb_message = rospy.wait_for_message("d405/color/image_rect_raw", Image)
        depth_message= rospy.wait_for_message("d405/aligned_depth_to_color/image_raw", Image)
        rgb_data = ros_numpy.numpify(rgb_message)
        depth_data = ros_numpy.numpify(depth_message)
        rgb_image = PIL.Image.fromarray(rgb_data)
        depth_image = PIL.Image.fromarray(depth_data)
        rgb_image.save(f"Misc/rgb_{self.count}.png")
        depth_image.save(f"Misc/depth_{self.count}.png")
        rospy.sleep(0.5
                    )
        self.count += 1
        with self.lock:
